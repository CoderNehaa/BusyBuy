import React from 'react';
import {Link, Outlet } from 'react-router-dom';
import { UserCustomHook } from './context/UserContext';

const Navbar = () => {
    const {user, logOut} = UserCustomHook();
  return (
    <>
      <div className="flex justify-between font-bold py-7 items-center shadow-lg shadow-white-10">
        <div className="w-5/12">
            <h1 className="pl-10 text-4xl "> 
                <Link to='/' className="linkStyle"> Busybuy </Link>   
            </h1>
        </div>

        <div className="w-7/12">
            <ul className="flex justify-evenly list-none text-xl">
                <li className="flex justify-center"> 
                    <img className="h-7 w-8 mr-2" src="https://www.nicepng.com/png/detail/13-131873_home-icons-png-transparent-home-icon-green-png.png" alt="home"/> 
                    <Link to='/' className="linkStyle"> Home </Link> 
                </li>
                
                { !user && <li className="flex justify-center"> 
                    <img className="h-7 w-8 mr-2" src="https://img.freepik.com/free-icon/login-symbol_318-9896.jpg" alt="sign-in"/> 
                    <Link to = '/signin' className="linkStyle"> Sign In </Link>
                </li>}

                {user && <li className="flex justify-center"> 
                    <img className="h-7 w-8 mr-2" src="https://www.pngmart.com/files/7/Cart-PNG-Clipart.png" alt="cart"/> 
                    <Link to='/cart' className="linkStyle"> Cart </Link>
                </li>}

                {user &&<li className="flex justify-center"> 
                    <img className="h-7 w-8 mr-2" src="data:image/png;base64," alt="cart"/> 
                    <Link to='/orders' className="linkStyle"> My Orders </Link>
                </li>}

                {user && <li className="flex justify-center">
                    <img className="h-7 w-8 mr-2" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSak-8ASizqk4vo_tPwfUTWsmXeaswsf1fWRDqupJJm4kOdK-FEpStQ5M9qqHpgzQZGW4I&usqp=CAU" alt="sign-out"/>
                    <Link to = '/' className="linkStyle" onClick={logOut}> Sign Out </Link>
                </li>}
                
            </ul>
        </div>
      </div>
      <Outlet />
      </>
  )
}

export default Navbar;
